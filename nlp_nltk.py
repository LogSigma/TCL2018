...
# 형태소 분류
train_docs = [(['/'.join(t) for t in twitter.pos(row[2], norm=True, stem=True)], row[3]) for row in train_data.itertuples()]
test_docs = [(['/'.join(t) for in twitter.pos(row[2], norm=True, stem=True)], row[3]) for row in test_data.itertuples()]
